...
-- 프로시저2 정의(만들기) : 사원번호가 100인 사원의 FIRST_NAME, LAST_NAME, SALARY 조회하는 프로시저
...
-- 프로시저3 정의(만들기) : 사원번호를 전달하면 해당 사원의 FIRST_NAME, LAST_NAME, SALARY 조회하는 프로시저
-- 입력 파라미터
...
BEGIN
    SELECT FIRST_NAME, LAST_NAME
      INTO FNAME, LNAME -- 출력  파라미터 값이 저장된다.
      FROM EMPLOYEES
     WHERE EMPLOYEE_ID = 100;
END;
...
DECLARE
    ...
    LNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    PROC4(FNAME, LNAME); -- PLSQL 내부에서 프로시저를 호출할 때는 EXECUT를 생략해야 한다.
    DBMS_OUTPUT.PUT_LINE(FNAME || ',' || LNAME);
END;

-- 프로시저5 사원번호를 전달하면, 해당 사원의 FIRST_NAME이 출력 파라미터 FNAME에 저장되도록 작성
-- 없는 사원번호가 전달되면 출력파라미터 FNAME에 'NoName'이 저장 되도록
-- 실행예시)
-- PROC6(100, FNAME) : 사원번호 100은 Sreven입니다.
-- PROC6(0, FNAME)   : 사원번호 0은 Noname입니다.
CREATE OR REPLACE PROCEDURE PROC5(EMP_ID IN EMPLOYEES.EMPLOYEE_ID%TYPE, 
