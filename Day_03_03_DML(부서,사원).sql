...
-- 참고. 자격증에서는 DML을 INSERT,UPDATE,DELETE + SELECT로 보는 경우도 있다. 자격증에서
...
CREATE SEQUENCE DEPARTMENT_SEQ
    ...
    ORDER           --  번호 건너뛰기가 없다.           NOORDER : 번호 건너뛰기가 있다.
;
    

-- 시퀀스에서 번호 뽑는 함수 : NEXTVAL
-- SELECT DEPARTMENT_SEQ.NEXTVAL FROM DUAL;    --  테이블에 없는 데이터를 조회 하려면 DUAL 테이블을 사용한다. 실제로 DUAL은 아무 의미 없다.
    --  위에 함수는 확인 후 실제에서는 사용 하지 말것.
...
INSERT INTO EMPLOYEE_TBL(EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY)
    VALUES(EMP_SEQ.NEXTVAL, '구창민', 1, '과장', 'M', '95/05/01', 5000000);   --  날짜 입력은 / 또는 - 로만 입력 가능
    INSERT INTO EMPLOYEE_TBL(EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY)
...
UPDATE DEPARTMENT_TBL 
   SET LOCATION = '경기'  --  수정 할 내용(여기서 등호는 대입 연산자)
 WHERE DEPT_NO = 1;       --  조건문(여기서 등호는 비교 연산자이다.)
COMMIT;

-- 2. 부서번호(DEPART)가 1인 부서에 근무하는 사원들의 급여(SALARY)를 500000원 증가 시키시오
UPDATE EMPLOYEE_TBL
...
-- 데이터 삭제하기
-- 1. 지역(LOCATION)이 '대구'인 부서를 삭제하시오. (대구에서 근무하는 사원이 없으므로 문제 없이 삭제된다.)
DELETE FROM DEPARTMENT_TBL WHERE LOCATION = '대구';
COMMIT;

-- 2. 지역(LOCATION)이 '서울'인 부서를 삭제하시오. ('서울'에서 근무하는 사원의 부서번호(DEPART)가 ON DELETE SET NULL 외래키 옵션에 의해서 NULL로 처리 된다.
