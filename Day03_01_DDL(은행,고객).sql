/*
    ...
    4. 완려된 작업을 취소 할 수 없다.(COMMIT 할 필요가 없다. ROLLBACK을 할 수 없다.)
    5. 종류
        1) CREATE : 생성
        2) ALTER  : 수정
        3) DROP   : 삭제
...
CREATE TABLE CUSTOMER_TBL(
     ...
     PHONE     VARCHAR2(30 BYTE) UNIQUE NULL,
...
-- 3. BANK_TBL 테이블의 은행명(BANK_NAME) 데이터 타입을 VARCHAR2(15 BYTE)으로 변경 하시오.
ALTER TABLE BANK_TBL
...
-- GRADE 칼럼은 'VIP' 'GOLD' 'SILVER', 'BRONZE' 중 하나의 값만 가질 수 있습니다.
ALTER TABLE CUSTOMER_TBL
...
ALTER TABLE CUSTOMER_TBL
    MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL;     -- 수정 할 때는 NULL을 입력 하지 않으면 기존 NOT NULL을 쓰겠다는 의미
                                                 -- 반드시 수정 할 때는 NULL 명시 해줘야 함
