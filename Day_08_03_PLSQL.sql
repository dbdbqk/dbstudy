...
/*
    테이블 복사하기
    1. CREATE TABLE과 복사할 데이터의 조회(SELECT)을 이용한다.
    2. PK와 FK 제약조건은 복사되지 않는다.
    3. 복사하는 쿼리
        1) 데이터를 복사하기
            CREATE TABLE 테이블 AS(SELECT 칼럼 FROM 테이블);
        2) 데이터를 제외하고 구조만 복사하기
            CREATE TABLE 테이블 AS(SELECT 칼럼 FROM 테이블 WHERE 1=2); -- WHERE 1=2 는 언제나 거짓인 조건이기 때문에 데이터를 안가져옴
            
*/
 

-- HR 계정의 EMPLOYEES 테이블을 GDJ61 계정으로 복사해서 기초데이터로 사용하기
DROP TABLE EMPLOYEES;
CREATE TABLE EMPLOYEES
    AS (SELECT *
          FROM HR.EMPLOYEES);
          
-- EMPLOYEES 테이블의 PK 생성하기
...
--    SELECT절의 INTO을 이용해서 테이블의 데이터를 가져와서 변수에 저장할 수 있다.
...
-- EMPLOYEE_ID가 150인 사원의COMMISSION_PCT가 0이면 '커미션없음', 아니면 실제 커미션(COMMISSION_PCT * SALARY)을 출력하시오.
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
...
DECLARE
    EMP EMPLOYEES%ROWTYPE;
BEGIN
     FOR EMP IN (SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID, SALARY, COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID FROM EMPLOYEES WHERE DEPARTMENT_ID = 50) LOOP
         INSERT INTO DEPT50 VALUES EMP;
    END LOOP;
    COMMIT;
END;
...
-- 1부터 정수 값을 누적하시오. 누적 값이 100을 초과하면 그만 누적하고 어디까지 누적 했는지 출력 하시오
DECLARE
    N NUMBER;
    TOTAL NUMBER;
BEGIN
    ...
    WHILE TRUE LOOP
        ...
        N := N + 1;
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('1 부터 ' || N || '까지 합은 ' || TOTAL || '입니다');
END;

-- 1부터 3의 배수를 제외한 정수 값을 누적하시오. 누적 같이 100을 초과하면 그만 누적하고 어디까지 누적했는지 출력하시오.
