...
/*
    ...
    2. 다중 행 서브쿼리
        ...
        3) 다중 행 서브쿼리에서 사용하는 연산자
            다중 행 연산자를 사용(IN, ANYM ALL 등)
*/

/* WHERE절의 서브쿼리 */

-- 1. 사원 번호가 1001인 사원과 동일한 직급(POSITION)을 가진 사원을 조회하시오.
SELECT 사원정보
  FROM 사원테이블
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE POSITION = (SELECT POSITION
                     FROM EMPLOYEE_TBL
                    WHERE EMP_NO = 1001); -- 단일행쿼리 ( EMP_NO는 PK)

-- 2. 부서번호가 2인 부서와 동일한 지역에 있는 부서를 조회 하시오.
SELECT 부서정보
  FROM 부서
...
--  6. 부서정보가 2인 부서에 근무하는 사원들의 직급과 일치하는 사원을 조회 하시오.
SELECT 사원정보
  FROM 사원
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE POSITION IN (SELECT POSITION
                    ...
                    WHERE DEPART = 2); -- 서브쿼리의 WHERE절에서 사용한 칼럼이 PK나 UNIQUE 칼럼이 아니므로 다중 행 서브쿼리이다.
                                       -- 따라서 단일 행 연사자인 등호(=) 대신 다중 행 연산자 IN을 사용해야 한다.
...
-- 9. '영업부'에서 가장 높은 급여보다 더 높은 급여를 받는 사원을 조회 하시오.
SELECT 사원정보
FROM 사원
WHERE 급여 > ('영업부'의 최대급여);
...
/*
    ...
    5. 인라인 뷰를 이용해서 SELECT문의 실행 순서를 조정 할 수 있다.
*/
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAFIUAABAAALDBAAA';

/*
    ROWNUM의 제약 사항
    1. ROWNUM이 1을 포함하는 범위를 조건으로 사용할 수 있다.
    2. ROWNUM이 1을 포함하지 않는 범위는 조건으로 사용할 수 없다.
    3. 모든 ROWNUN을 사용하려면 ROWNUM에 별명을 지정하고 그 별명을 사용하면 된다.
...
SELECT E.EMP_NO, E.NAME, E.DEPART
  FROM (SELECT EMP_NO, NAME, DEPART
          FROM EMPLOYEE_TBL
         ORDER BY EMP_NO) E; -- 프롬절의 서브쿼리는 작성 후 별명을 줘야함


-- 1. 연봉이 2번째로 높은 사원을 조회하시오.
--      1) 연봉순으로 정렬한다
--      2) 정렬 결과에 행 번호(ROWNUM)을 붙이낟.
--      3) 원하는 행 번호를 조회한다.
-- 1) ROWNUM 칼럼 사용하기
SELECT E.EMP_NO, E.NAME, E  .DEPART, E.GENDER, E.POSITION, E.HIRE_DATE, E.SALARY
...
/*
    스칼라 서브쿼리
    1. SELECT절에서하나의 값을 반환하는 서브쿼리
    2. 일치하지 않는 정보는 NULL값을 반환한다.
    3. 유사한 방식의 조인 방식은 외부조인이다.
*/

-- 부서번호가 1인 부서에 근무하는 사원번호, 사원명, 부서번호, 부서명을 근무하는 사원을 조회하시오.
SELECT
       E.EMP_NO
     , E.NAME
     , E.DEPART
